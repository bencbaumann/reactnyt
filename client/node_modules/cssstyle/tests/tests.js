...
module.exports = {
  ...
  'Test From Style String': function (test) {
    ...
    test.ok(style.length === 3, 'length is not 3')
    test.ok(style.cssText === 'color: blue; background-color: red; width: 78%;', 'cssText is wrong')
    test.ok(style.getPropertyValue('color') === 'blue', "getPropertyValue('color') failed")
    test.ok(style.item(0) === 'color', 'item(0) failed')
    test.ok(style[1] === 'background-color', 'style[1] failed')
    test.ok(style.backgroundColor === 'red', 'style.backgroundColor failed with "' + style.backgroundColor + '"')
    style.cssText = ''
  ...
  'Test From Properties': function (test) {
    ...
    test.ok(style[0] === 'color', 'style[0] is not color')
    test.ok(style.cssText === 'color: blue;', 'cssText is wrong')
    test.ok(style.item(0) === 'color', 'item(0) is not color')
    test.ok(style.color === 'blue', 'color is not blue')
    style.backgroundColor = 'red'
    test.ok(style.length === 2, 'length is not 2')
    test.ok(style[0] === 'color', 'style[0] is not color')
    test.ok(style[1] === 'background-color', 'style[1] is not background-color')
    test.ok(style.cssText === 'color: blue; background-color: red;', 'cssText is wrong')
    test.ok(style.backgroundColor === 'red', 'backgroundColor is not red')
    style.removeProperty('color')
    test.ok(style[0] === 'background-color', 'style[0] is not background-color')
    test.done()
  },
  'Test Shorthand Properties': function (test) {
    ...
    test.ok(style.backgroundColor === 'blue', 'backgroundColor is not blue')
    test.ok(style.backgroundImage === 'url(http://www.example.com/some_img.jpg)', 'backgroundImage is wrong')
    test.ok(style.background === 'blue url(http://www.example.com/some_img.jpg)', 'background is different')
    style.border = '0 solid black'
    test.ok(style.borderWidth === '0px', 'borderWidth is not 0px')
    test.ok(style.borderStyle === 'solid', 'borderStyle is not solid')
    test.ok(style.borderColor === 'black', 'borderColor is not black')
    test.ok(style.borderTopWidth === '0px', 'borderTopWidth is not 0px')
    test.ok(style.borderLeftStyle === 'solid', 'borderLeftStyle is not solid')
    test.ok(style.borderBottomColor === 'black', 'borderBottomColor is not black')
    style.font = '12em monospace'
    test.ok(style.fontSize === '12em', 'fontSize is not 12em')
    test.ok(style.fontFamily === 'monospace', 'fontFamily is not monospace')
    test.done()
  },
  'Test width and height Properties and null and empty strings': function (test) {
    ...
    style.width = 0
    test.ok(style.width === '0px', 'width is not 0px')
    style.height = '34%'
    test.ok(style.height === '34%', 'height is not 34%')
    style.height = ''
    test.ok(style.length === 1, 'length is not 1')
    test.ok(style.cssText === 'width: 0px;', 'cssText is not "width: 0px;"')
    style.width = null
    test.ok(style.length === 0, 'length is not 0')
  ...
  'Test Implicit Properties': function (test) {
    ...
    test.ok(style.borderWidth === '0px', 'borderWidth is not 0px')
    test.ok(style.borderTopWidth === '0px', 'borderTopWidth is not 0px')
    test.ok(style.borderBottomWidth === '0px', 'borderBottomWidth is not 0px')
    test.ok(style.borderLeftWidth === '0px', 'borderLeftWidth is not 0px')
    test.ok(style.borderRightWidth === '0px', 'borderRightWidth is not 0px')
    test.ok(style.cssText === 'border-width: 0px;', 'cssText is not "border-width: 0px", "' + style.cssText + '"')
    test.done()
  },
  'Test Top, Left, Right, Bottom Properties': function (test) {
    ...
    style.right = '5em'
    style.bottom = '12pt'
    test.ok(style.top === '0px', 'top is not 0px')
    test.ok(style.left === '0%', 'left is not 0%')
    test.ok(style.right === '5em', 'right is not 5em')
    test.ok(style.bottom === '12pt', 'bottom is not 12pt')
    test.ok(style.length === 4, 'length is not 4')
    test.ok(style.cssText === 'top: 0px; left: 0%; right: 5em; bottom: 12pt;', 'text is not "top: 0px; left: 0%; right: 5em; bottom: 12pt;"')
    test.done()
  },
  'Test Clear and Clip Properties': function (test) {
    ...
    test.expect(10)
    style.clear = 'none'
    test.ok(style.clear === 'none', 'clear is not none')
    style.clear = 'lfet' // intentionally wrong
    test.ok(style.clear === 'none', 'clear is not still none')
    style.clear = 'left'
    test.ok(style.clear === 'left', 'clear is not left')
    style.clear = 'right'
    test.ok(style.clear === 'right', 'clear is not right')
    style.clear = 'both'
    test.ok(style.clear === 'both', 'clear is not both')
    style.clip = 'elipse(5px, 10px)'
    test.ok(style.clip === '', 'clip should not be set')
    test.ok(style.length === 1, 'length is not 1')
    style.clip = 'rect(0, 3Em, 2pt, 50%)'
    test.ok(style.clip === 'rect(0px, 3em, 2pt, 50%)', 'clip is not "rect(0px, 3em, 2pt, 50%)", "' + style.clip + '"')
    test.ok(style.length === 2, 'length is not 2')
    test.ok(style.cssText === 'clear: both; clip: rect(0px, 3em, 2pt, 50%);', 'cssText is not "clear: both; clip: rect(0px, 3em, 2pt, 50%);"')
    test.done()
  },
  'Test colors': function (test) {
    ...
    test.ok(style.color === 'rgba(0, 0, 0, 0)', 'color is not rgba(0, 0, 0, 0)')
    style.color = 'rgba(5%, 10%, 20%, 0.4)'
    test.ok(style.color === 'rgba(12, 25, 51, 0.4)', 'color is not rgba(12, 25, 51, 0.4)')
    style.color = 'rgb(33%, 34%, 33%)'
    test.ok(style.color === 'rgb(84, 86, 84)', 'color is not rgb(84, 86, 84)')
    style.color = 'rgba(300, 200, 100, 1.5)'
    test.ok(style.color === 'rgb(255, 200, 100)', 'color is not rgb(255, 200, 100) ' + style.color)
    test.done()
  },
  'Test short hand properties with embedded spaces': function (test) {
    ...
    test.ok(style.backgroundColor === 'rgb(0, 0, 0)', 'backgroundColor is not rgb(0, 0, 0): ' + style.backgroundColor)
    test.ok(style.backgroundImage === 'url(/something/somewhere.jpg)', 'backgroundImage is not url(/something/somewhere.jpg): ' + style.backgroundImage)
    test.ok(style.cssText === 'background: rgb(0, 0, 0) url(/something/somewhere.jpg);', 'cssText is not correct: ' + style.cssText)
    style = new cssstyle.CSSStyleDeclaration()
    style.border = '  1px  solid   black  '
    test.ok(style.border === '1px solid black', 'multiple spaces not properly parsed: ' + style.border)
    test.done()
  },
  'Setting shorthand properties to an empty string should clear all dependent properties': function (test) {
    ...
    test.ok(style.cssText === 'border-width: 1px;', 'cssText is not "border-width: 1px;": ' + style.cssText)
    style.border = ''
  ...
  'Setting implicit properties to an empty string should clear all dependent properties': function (test) {
    ...
    test.ok(style.cssText === 'border-top-width: 1px;', 'cssText is not "border-top-width: 1px;": ' + style.cssText)
    style.borderWidth = ''
    test.ok(style.cssText === '', 'cssText is not "": ' + style.cssText)
    test.done()
  },
  'Setting a shorthand property, whose shorthands are implicit properties, to an empty string should clear all dependent properties': function (test) {
    ...
    test.ok(style.cssText === 'border-top: 1px solid black;', 'cssText is not "border-top: 1px solid black;": ' + style.cssText)
    style.border = ''
    test.ok(style.cssText === '', 'cssText is not "": ' + style.cssText)
    test.done()
  },
  'Setting border values to "none" should clear dependent values': function (test) {
    ...
    test.ok(style.cssText === 'border-top-width: 1px; border-left-width: 1px;', 'cssText is not "border-top-width: 1px; border-left-width: 1px;": ' + style.cssText)
    style.borderTop = 'none'
    test.ok(style.cssText === 'border-left-width: 1px;', 'cssText is not "border-left-width: 1px;": ' + style.cssText)
    test.done()
  },
  'Setting border to 0 should be okay': function (test) {
    ...
    test.expect(1)
    style.border = 0
    test.ok(style.cssText === 'border: 0px;', 'cssText is not "border: 0px;": ' + style.cssText)
    test.done()
  },
  'Setting values implicit and shorthand properties via cssText and setProperty should propagate to dependent properties': function (test) {
    ...
    test.ok(style.cssText === 'border: 1px solid black;', 'cssText is not "border: 1px solid black;": ' + style.cssText)
    test.ok(style.borderTop === '1px solid black', 'borderTop is not "1px solid black": ' + style.borderTop)
    style.border = ''
    test.ok(style.cssText === '', 'cssText is not "": ' + style.cssText)
  ...
  'Setting opacity should work': function (test) {
    ...
    test.ok(style.cssText === 'opacity: 0.75;', 'cssText is not "opacity: 0.75;": ' + style.cssText)
    style.opacity = '0.50'
    test.ok(style.cssText === 'opacity: 0.5;', 'cssText is not "opacity: 0.5;": ' + style.cssText)
    style.opacity = 1.0
    test.ok(style.cssText === 'opacity: 1;', 'cssText is not "opacity: 1;": ' + style.cssText)
    test.done()
  },
  ...
  'Setting a value to 0 should return the string value': function (test) {
    ...
    test.ok(style.fillOpacity === '0', 'fillOpacity is not "0": ' + style.fillOpacity)
    test.done()
  },
  'onChange callback should be called when the cssText changes': function (test) {
    var style = new cssstyle.CSSStyleDeclaration(function (cssText) {
      test.ok(cssText === 'opacity: 0;', 'cssText is not "opacity: 0;": ' + cssText)
      test.done()
    })
    test.expect(1)
  ...
  'Setting float should work the same as cssFloat': function (test) {
    ...
    test.expect(1)
    style.float = 'left'
    test.ok(style.cssFloat === 'left', 'cssFloat is not "left": ' + style.cssFloat)
    test.done()
  },
  ...
  'Make sure url parsing works with quotes': function (test) {
    ...
    test.ok(style.backgroundImage === 'url(http://some/url/here1.png)', 'background-image wasn\'t url(http://some/url/here1.png): ' + style.backgroundImage)
    style.backgroundImage = 'url(\'http://some/url/here2.png\')'
    test.ok(style.backgroundImage === 'url(http://some/url/here2.png)', 'background-image wasn\'t url(http://some/url/here2.png): ' + style.backgroundImage)
    style.backgroundImage = 'url("http://some/url/here3.png")'
    test.ok(style.backgroundImage === 'url(http://some/url/here3.png)', 'background-image wasn\'t url(http://some/url/here3.png): ' + style.backgroundImage)
    test.done()
  },
