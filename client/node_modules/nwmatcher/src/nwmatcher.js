...
(function (global, factory) {
  ...
  var version = 'nwmatcher-1.4.3',
    ...
    function (data, elements) {
      var i = -1, element
      if (!data.length && Array.slice) { return Array.slice(elements) }
      while ((element = elements[++i])) { data[data.length] = element }
      return data
    },
    ...
    function (data, elements, callback) {
      ...
      while ((element = elements[++i])) {
        if (callback(data[data.length] = element) === false) { break }
      }
      return data
    },
    ...
    function (str) {
      return str.replace(reEscapedChars,
        function (substring, p1, p2) {
          // unescaped " or '
          return p2 || (/^[0-9a-fA-F]/.test(p1) ? stringFromCodePoint(parseInt(p1, 16))
          // \' \"
            : /^[\\\x22\x27]/.test(p1) ? substring
              // \g \h \. \# etc
              : p1)
