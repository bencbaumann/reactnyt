...
TokenizerProxy.prototype._ensureTokenizerMode = function (tn) {
  if (tn === $.TEXTAREA || tn === $.TITLE) { this.tokenizer.state = Tokenizer.MODE.RCDATA } else if (tn === $.PLAINTEXT) { this.tokenizer.state = Tokenizer.MODE.PLAINTEXT } else if (tn === $.SCRIPT) { this.tokenizer.state = Tokenizer.MODE.SCRIPT_DATA } else if (tn === $.STYLE || tn === $.IFRAME || tn === $.XMP ||
...
TokenizerProxy.prototype._handleStartTagToken = function (token) {
  ...
  if (tn === $.SVG) { this._enterNamespace(NS.SVG) } else if (tn === $.MATH) { this._enterNamespace(NS.MATHML) } else {
    if (this.inForeignContent) {
      if (ForeignContent.causesExit(token)) { this._leaveCurrentNamespace() } else if (ForeignContent.isMathMLTextIntegrationPoint(tn, this.currentNamespace) ||
  ...
  if (!this.inForeignContent) {
    ...
    if (ForeignContent.isMathMLTextIntegrationPoint(tn, previousNs) ||
      ...
      this._leaveCurrentNamespace()
             ...
             (tn === $.MATH && this.currentNamespace === NS.MATHML)) { this._leaveCurrentNamespace() }
}
