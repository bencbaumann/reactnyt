...
const equalPackages = {
  ...
  'assert.deepStrictEqual': (a, b) => {
    try { assertDeepStrictEqual(a, b); return true } catch (e) { return false }
  }
}
...
for (const equalName in equalPackages) {
  ...
  for (const testSuite of tests) {
    for (const test of testSuite.tests) {
      try {
        if (equalFunc(test.value1, test.value2) !== test.equal) { console.error('different result', equalName, testSuite.description, test.description) }
      } catch (e) {
  ...
  suite.add(equalName, function () {
    for (const testSuite of tests) {
      for (const test of testSuite.tests) {
        if (test.description != 'pseudo array and equivalent array are not equal') { equalFunc(test.value1, test.value2) }
      }
    }
  })
}
