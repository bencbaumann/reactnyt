...
function Handle (stream, options) {
  ...
  if (mode === 'rusty') { this.writeQueueSize = 0 } else if (mode !== 'modern') { this.writeQueueSize = 1 }

  if (mode === 'rusty') {
    if (this._stream) { this._rustyInit() } else { this.once('stream', this._rustyInit) }
  }
}
...
if (mode !== 'rusty') {
  Handle.prototype.close = function close (callback) {
    ...
    if (this._options.close) { this._options.close(callback) } else { process.nextTick(callback) }
  ...
  Handle.prototype.close = function close () {
    ...
    if (!this._stream) { this.once('stream', this.close) } else if (this._options.close) { this._options.close(function () {}) }
...
Handle.prototype.getsockname = function getsockname () {
  if (this._options.getPeerName) { return this._options.getPeerName() }
  return null
}
