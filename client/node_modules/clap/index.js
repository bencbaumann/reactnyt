  ...
  if (arguments.length == 3) {
    if (opt_1 && opt_1.constructor === Object) {
      for (var key in opt_1) {
        if (key == 'normalize' ||
            key == 'defValue' ||
            key == 'beforeInit') { option[key] = opt_1[key] }
      }
...
function processArgs (command, args, suggest) {
  function processOption (option, command) {
    ...
    if (option.maxArgsCount) {
      ...
      if (option.maxArgsCount == 1) { params = params[0] }
    } else {
...
Command.prototype = {
    ...
    option.normalize = function (value) {
      ...
      for (var name in values) {
        if (hasOwnProperty.call(values, name)) {
          if (hasOwnProperty.call(command.options, name)) { command.setOption(name, values[name]) } else { command.values[name] = values[name] }
        }
      }
  ...
  setOptions: function (values) {
    for (var name in values) {
      if (hasOwnProperty.call(values, name) && this.hasOption(name)) { this.setOption(name, values[name]) }
    }
  },
  reset: function () {
    this.values = {}
  ...
  parse: function (args, suggest) {
    ...
    if (!errorHandler) { return processArgs(this, args, suggest) } else {
      try {
  ...
  run: function (args, context) {
    ...
    for (var i = 0; i < commands.length; i++) {
      ...
      item.options.forEach(function (entry) {
        if (entry.option.beforeInit) { command.setOption(entry.option.camelName, entry.value) }
      })

      command.init_(item.args.slice()) // use slice to avoid args mutation in handler

      if (item.args.length) { command.args_(item.args.slice()) } // use slice to avoid args mutation in handler
  ...
  normalize: function (values) {
    ...
    for (var name in values) {
      if (hasOwnProperty.call(values, name) && !hasOwnProperty.call(result, name)) { result[name] = values[name] }
    }
