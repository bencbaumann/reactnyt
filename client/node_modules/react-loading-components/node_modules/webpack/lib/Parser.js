      ...
      for (let specifierIndex = 0; specifierIndex < statement.specifiers.length; specifierIndex++) {
        ...
        switch (specifier.type) {
          case 'ExportSpecifier':
          {
            ...
            if (source) { this.applyPluginsBailResult('export import specifier', statement, source, specifier.local.name, name, specifierIndex) } else { this.applyPluginsBailResult('export specifier', statement, specifier.local.name, name, specifierIndex) }
            break
          }
        }
      }
    ...
    function walkIIFE (functionExpression, options, currentThis) {
      function renameArgOrThis (argOrThis) {
        ...
        if (renameIdentifier && this.applyPluginsBailResult1('can-rename ' + renameIdentifier, argOrThis)) {
          if (!this.applyPluginsBailResult1('rename ' + renameIdentifier, argOrThis)) { return renameIdentifier }
        }
