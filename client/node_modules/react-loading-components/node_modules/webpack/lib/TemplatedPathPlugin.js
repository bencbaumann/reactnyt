  ...
  apply (compiler) {
    compiler.plugin('compilation', compilation => {
      ...
      mainTemplate.plugin('hash-for-chunk', function (hash, chunk) {
        const outputOptions = this.outputOptions
        const chunkFilename = outputOptions.chunkFilename || outputOptions.filename
        if (REGEXP_CHUNKHASH_FOR_TEST.test(chunkFilename)) { hash.update(JSON.stringify(chunk.getChunkMaps(false, true).hash)) }
        if (REGEXP_NAME_FOR_TEST.test(chunkFilename)) { hash.update(JSON.stringify(chunk.getChunkMaps(false, true).name)) }
      })
    })
  }
}
