...
class CachePlugin {
          ...
          if (!compilation.notCacheable) {
            ...
            compilation.plugin('child-compiler', (childCompiler, compilerName, compilerIndex) => {
              if (cache) {
                ...
                if (cache.children[compilerName][compilerIndex]) { childCache = cache.children[compilerName][compilerIndex] } else { cache.children[compilerName].push(childCache = {}) }
  ...
  applyMtime (mtime) {
    if (this.FS_ACCURENCY > 1 && mtime % 2 !== 0) { this.FS_ACCURENCY = 1 } else if (this.FS_ACCURENCY > 10 && mtime % 20 !== 0) { this.FS_ACCURENCY = 10 } else if (this.FS_ACCURENCY > 100 && mtime % 200 !== 0) { this.FS_ACCURENCY = 100 } else if (this.FS_ACCURENCY > 1000 && mtime % 2000 !== 0) { this.FS_ACCURENCY = 1000 }
  }
}
