(function (global, factory) {
  ...
  pp$1.parseStatement = function (declaration, topLevel, exports) {
    ...
    switch (starttype) {
      ...
      default:
        ...
        if (starttype === types.name && expr.type === 'Identifier' && this.eat(types.colon)) { return this.parseLabeledStatement(node, maybeName, expr) } else { return this.parseExpressionStatement(node, expr) }
    }
  }
  ...
  pp$1.parseBreakContinueStatement = function (node, keyword) {
    ...
    this.next()
    if (this.eat(types.semi) || this.insertSemicolon()) { node.label = null } else if (this.type !== types.name) { this.unexpected() } else {
  ...
  pp$1.parseDoStatement = function (node) {
    ...
    if (this.options.ecmaVersion >= 6) { this.eat(types.semi) } else { this.semicolon() }
  ...
  pp$1.parseReturnStatement = function (node) {
    ...
    if (this.eat(types.semi) || this.insertSemicolon()) { node.argument = null } else { node.argument = this.parseExpression(); this.semicolon() }
  ...
  pp$1.parseForIn = function (node, init) {
    ...
    if (type == 'ForInStatement') {
      ...
      (init.type === 'VariableDeclaration' && init.declarations[0].init != null &&
       (this.strict || init.declarations[0].id.type !== 'Identifier'))) { this.raise(init.start, 'Invalid assignment in for-in loop head') }
    }
    node.left = init
  ...
  pp$1.parseFunction = function (node, isStatement, allowExpressionBody, isAsync) {
    ...
    if (this.options.ecmaVersion >= 9 || this.options.ecmaVersion >= 6 && !isAsync) { node.generator = this.eat(types.star) }
    ...
    if (!isStatement) { node.id = this.type == types.name ? this.parseIdent() : null }
  ...
  pp$2.parseBindingList = function (close, allowEmpty, allowTrailingComma) {
    ...
    while (!this.eat(close)) {
      if (first) { first = false } else { this$1.expect(types.comma) }
  ...
  pp$3.parseMaybeUnary = function (refDestructuringErrors, sawUnary) {
    ...
    if (!sawUnary && this.eat(types.starstar)) { return this.buildBinary(startPos, startLoc, expr, this.parseMaybeUnary(null, false), '**', false) } else { return expr }
  }
  ...
  pp$3.parseExprAtom = function (refDestructuringErrors) {
    ...
    switch (this.type) {
      ...
      case types.name:
        ...
        if (this.options.ecmaVersion >= 8 && !containsEsc && id.name === 'async' && !this.canInsertSemicolon() && this.eat(types._function)) { return this.parseFunction(this.startNodeAt(startPos, startLoc), false, false, true) }
  ...
  pp$3.parseNew = function () {
    ...
    if (this.eat(types.parenL)) { node.arguments = this.parseExprList(types.parenR, this.options.ecmaVersion >= 8, false) } else { node.arguments = empty$1 }
  ...
  pp$3.parseExprList = function (close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
    ...
    while (!this.eat(close)) {
      ...
      var elt = (void 0)
      if (allowEmpty && this$1.type === types.comma) { elt = null } else if (this$1.type === types.ellipsis) {
        elt = this$1.parseSpread(refDestructuringErrors)
        if (refDestructuringErrors && this$1.type === types.comma && refDestructuringErrors.trailingComma < 0) { refDestructuringErrors.trailingComma = this$1.start }
      } else {
 ...
 */
  ...
  pp$7.updateContext = function (prevType) {
    ...
    if (type.keyword && prevType == types.dot) { this.exprAllowed = false } else if (update = type.updateContext) { update.call(this, prevType) } else { this.exprAllowed = type.beforeExpr }
  }
  ...
  types._function.updateContext = types._class.updateContext = function (prevType) {
    if (prevType.beforeExpr && prevType !== types.semi && prevType !== types._else &&
      !((prevType === types.colon || prevType === types.braceL) && this.curContext() === types$1.b_stat)) { this.context.push(types$1.f_expr) } else { this.context.push(types$1.f_stat) }
    this.exprAllowed = false
  }

  types.backQuote.updateContext = function () {
    if (this.curContext() === types$1.q_tmpl) { this.context.pop() } else { this.context.push(types$1.q_tmpl) }
    this.exprAllowed = false
  }
  ...
  types.star.updateContext = function (prevType) {
    if (prevType == types._function) {
      ...
      if (this.context[index] === types$1.f_expr) { this.context[index] = types$1.f_expr_gen } else { this.context[index] = types$1.f_gen }
    }
    this.exprAllowed = true
  }
