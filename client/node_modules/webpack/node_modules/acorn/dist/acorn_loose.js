(function (global, factory) {
  ...
  lp$1.parseStatement = function () {
    ...
    switch (starttype) {
      ...
      case __acorn.tokTypes._return:
        this.next()
        if (this.eat(__acorn.tokTypes.semi) || this.canInsertSemicolon()) { node.argument = null } else { node.argument = this.parseExpression(); this.semicolon() }
  ...
  lp$1.parseClass = function (isStatement) {
    ...
    var node = this.startNode()
    this.next()
    if (this.tok.type === __acorn.tokTypes.name) { node.id = this.parseIdent() } else if (isStatement === true) { node.id = this.dummyIdent() } else { node.id = null }
  ...
  lp$1.parseFunction = function (node, isStatement, isAsync) {
    ...
    if (this.tok.type === __acorn.tokTypes.name) { node.id = this.parseIdent() } else if (isStatement === true) { node.id = this.dummyIdent() }
