...
class Compilation extends Tapable {
  ...
  buildModule (module, optional, origin, dependencies, thisCallback) {
    ...
    module.build(this.options, this, this.resolvers.normal, this.inputFileSystem, (error) => {
      ...
      for (let indexError = 0; indexError < errors.length; indexError++) {
        ...
        if (optional) { this.warnings.push(err) } else { this.errors.push(err) }
      }
  ...
  applyModuleIds () {
    ...
    for (let indexModule2 = 0; indexModule2 < modules2.length; indexModule2++) {
      ...
      if (module2.id === null) {
        if (unusedIds.length > 0) { module2.id = unusedIds.pop() } else { module2.id = nextFreeModuleId++ }
      }
    }
  }
  ...
  applyChunkIds () {
    ...
    for (let indexChunk = 0; indexChunk < chunks.length; indexChunk++) {
      ...
      if (chunk.id === null) {
        if (unusedIds.length > 0) { chunk.id = unusedIds.pop() } else { chunk.id = nextFreeChunkId++ }
      }
      if (!chunk.ids) {
        chunk.ids = [chunk.id]
      }
