...
class NormalModuleFactory extends Tapable {
  constructor (context, resolvers, options) {
    ...
    this.plugin('resolver', () => (data, callback) => {
      ...
      asyncLib.parallel([
        ...
        result.forEach(r => {
          if (r.type === 'use') {
            if (r.enforce === 'post' && !noPostAutoLoaders && !noPrePostAutoLoaders) { useLoadersPost.push(r.value) } else if (r.enforce === 'pre' && !noPrePostAutoLoaders) { useLoadersPre.push(r.value) } else if (!r.enforce && !noAutoLoaders && !noPrePostAutoLoaders) { useLoaders.push(r.value) }
          } else {
  ...
  getParser (parserOptions) {
    let ident = 'null'
    if (parserOptions) {
      if (parserOptions.ident) { ident = parserOptions.ident } else { ident = JSON.stringify(parserOptions) }
    }
