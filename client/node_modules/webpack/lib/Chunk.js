...
class Chunk {
  ...
  checkConstraints () {
    ...
    chunk.chunks.forEach((child, idx) => {
      if (chunk.chunks.indexOf(child) !== idx) { throw new Error(`checkConstraints: duplicate child in chunk ${chunk.debugId} ${child.debugId}`) }
      if (child.parents.indexOf(chunk) < 0) { throw new Error(`checkConstraints: child missing parent ${chunk.debugId} -> ${child.debugId}`) }
    })
    chunk.parents.forEach((parentChunk, idx) => {
      if (chunk.parents.indexOf(parentChunk) !== idx) { throw new Error(`checkConstraints: duplicate parent in chunk ${chunk.debugId} ${parentChunk.debugId}`) }
      if (parentChunk.chunks.indexOf(chunk) < 0) { throw new Error(`checkConstraints: parent missing child ${parentChunk.debugId} <- ${chunk.debugId}`) }
    })
  }
}
